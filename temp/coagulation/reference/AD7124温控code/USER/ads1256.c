...
*/
	...
	#define RCC_PWDN 	RCC_AHB1Periph_GPIOC
	#define PORT_PWDN	GPIOC
	#define PIN_PWDN	GPIO_Pin_4
...
static void switchChannel(uint8_t _ch)
{
	if(g_tADS1256.model == DIFF_e)
	{
...
static int32_t ADS1256_ReadAverData(void)
{
	...
	for(i=0;i<10;i++)
	{
		sum += ADS1256_ReadData();
	}
	
	
	AdcDataRev = sum/10 + 0xc7;
	return AdcDataRev;
}
...
float ADS1256_GetAdc(uint8_t _ch)
{
	...
	re = ((int64_t)iTemp * 2500000) / 4194303;
	return re;
}

float ADS1256_GetAdcAver()
{ 
	int32_t iTemp;
	float re =0;
	
	iTemp = ADS1256_ReadAverData();
...
{
	/* 读取采集结构，保存在全局变量 */					
	switchChannel(g_tADS1256.Channel);	/* 切换模拟通道 */	
	delay_us(5);
	...
	{
		g_tADS1256.AdcNow[g_tADS1256.MaxChannel -1] = ADS1256_GetAdcAver();	/* 注意保存的是上一个通道的数据 */
	}
	else
	{
		g_tADS1256.AdcNow[g_tADS1256.Channel-1] = ADS1256_GetAdcAver();	/* 注意保存的是上一个通道的数据 */
	}
				
	if (++g_tADS1256.Channel >= g_tADS1256.MaxChannel)
	{
		g_tADS1256.Channel = 0;
	}
}
