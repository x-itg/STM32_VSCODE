...
//#define Pt100_Channel_3Wire
#define Pt100_Channel_4Wire
#define Thermocouple
...
#define AD7124_CS_H GPIO_SetBits(GPIOB,GPIO_Pin_3)
#define AD7124_CS_L GPIO_ResetBits(GPIOB,GPIO_Pin_3)

#define AD7124_SCLK_H GPIO_SetBits(GPIOB,GPIO_Pin_4)
#define AD7124_SCLK_L GPIO_ResetBits(GPIOB,GPIO_Pin_4)

#define AD7124_DIN_H GPIO_SetBits(GPIOB,GPIO_Pin_5)
#define AD7124_DIN_L GPIO_ResetBits(GPIOB,GPIO_Pin_5)

#define AD7124_DOUT GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_6)
...
extern float Actual_Voltage[8],TH_voltage;
extern uint8_t DATA_STATUS;
extern uint8_t AD_Gain;
extern uint8_t AD_Onetime_Trig,Samprate,ad_start_working;
...
uint16_t AD7124_Write_Reg(uint8_t addr,uint8_t byte,uint32_t data);//写寄存器
void AD7124_SINGLE_INIT(uint8_t SamHz);//单通道初始化函数
void AD7124_MUL_INIT(uint8_t SamHz);//多通道初始化函数
void Get_Once_Vol(float *value);//采集一次电压值
